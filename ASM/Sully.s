...
%macro SRC_CHECK 2     ; Check if need to decrement or execute
    ...
    mov   rdi, r13
    mov   esi, '/'
    call  strrchr wrt ..plt ; strrchr(bin_name_full, '/')
    test  rax, rax
    jz    .no_path
...
.check_name:
    mov   esi, '_'
    call  strchr wrt ..plt  ; strchr(bin_name, '_')
    test  rax, rax
    jz    .check_var
    sub   %1, 1
